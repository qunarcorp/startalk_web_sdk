...
function image(beforeFn, successFn, progressFn, filesList, onlyUrl) {
  const { myId, key } = this;
  const url = sdkConfig.fileurl+`/file/v2/upload/img?size=48&u=${myId}&k=${key}`;
    ...
    add: (e, data) => {
      data.process().done(() => {
        $.each(data.files, (index, { size, name }) => {
          ...
          const newUrl = sdkConfig.fileurl+`/file/v2/upload/img?${paramLink}`;
          ...
          const checkFileUrl = sdkConfig.fileurl+`/file/v2/inspection/img?${paramLink}`;
...
function file(beforeFn, successFn, progressFn, filesList, onlyUrl) {
  const { myId, key } = this;
  const url = sdkConfig.fileurl+`/file/v2/upload/file?size=46&u=${myId}&k=${key}`;
    ...
    add: (e, data) => {
      data.process().done(() => {
        $.each(data.files, (index, { size, name }) => {
          ...
          const sizeMB = bytesToMB(size);
          const paramLink = `name=${name}&size=${sizeMB}&u=${myId}&k=${key}&key=${uuid}&p=qim_web`;
          const newUrl = sdkConfig.fileurl+`/file/v2/upload/file?${paramLink}`;
          if (size > limitFileSize) {
            alert('图片大小不能超过50M');
            return;
          }
          ...
          // 校验上传的文件是否存在
          // 如果文件存在了就不上传了，直接显示为和上传成功的效果
          const checkFileUrl = sdkConfig.fileurl+`/file/v2/inspection/file?${paramLink}`;
          checkUpLoadFileExist(checkFileUrl)
            .done(async (res) => {
              beforeFn();
              if (res.ret) {
